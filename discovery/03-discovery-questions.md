...
## About the Real Business Goal

**Q33: What decision do you want the Director to make from this report?**
*Assumed answer:* Primarily: which projects need immediate intervention before the QBR — whether that means reallocating resources, confronting a PM, escalating to the CEO, or recommending a contract extension. Secondarily: which systemic patterns need a process-level fix, not a project-level one.
*Impact on solution:* The report must be structured around decisions, not descriptions. Every flagged item should make it obvious what the Director's next move is — not just what happened. This is the difference between a report and a decision support tool.
...
*Impact on solution:* The flag taxonomy (stalled decisions, scope changes, incidents) maps directly to these three dimensions. The report structure should make this mapping visible so the Director knows which dimension each flag is touching.
...
*Impact on solution:* The system should optimise for recall over precision for high-severity flags — it is better to surface a false positive that a PM quickly dismisses than to miss a real issue. Lower-severity categories can apply stricter filtering to manage noise.
...
*Assumed answer:* Jira should be the authoritative source for resolution status (a closed ticket means closed). Email is the authoritative source for context, nuance, and issues that were never ticketed. If an issue appears resolved in Jira but the email thread shows ongoing escalation, that contradiction is itself a flag worth surfacing.
*Impact on solution:* Conflict resolution logic must be explicit. A Jira-first system does not blindly trust Jira — it treats email-Jira contradictions as a data quality signal.

**Q37: Do your teams consistently use email subjects, threads, and recipients in a structured way?**
*Assumed answer:* Partially — project name appears in most subjects, but threads drift (wrong-thread replies, forwarded chains, misdirected messages). The sample data confirms this: 14% of emails are off-topic, and several threads contain messages addressed to the wrong project.
*Impact on solution:* Parser accuracy is directly proportional to team email discipline. Low-structure environments will produce more false positives and more unknown project assignments. The system should log and surface these ambiguous cases rather than silently failing.

---

## About Actionability

**Q38: Should the report highlight trends over time — e.g., repeated scope creep or deployment failures across multiple quarters?**
*Assumed answer:* Yes — this is high value for the Director. A project that has had scope creep in three consecutive quarters has a process problem, not a project problem.
...
## About Rollout and Success

**Q39: How will you judge success of the PoC?**
*Assumed answer:* The PoC is successful if it correctly surfaces the issues a human reviewer would identify in the sample email data, with a false negative rate below 20% for high-severity flags. For production: QBR prep time drops from 4 hours to under 30 minutes, and the Director reports at least one issue surfaced that they would not have caught manually.
*Impact on solution:* These criteria should be agreed before delivery, not after. Without a concrete success metric, evaluation is subjective.

**Q40: Who will validate whether a flagged issue was genuinely important — and when?**
...
**Q41: What is the minimum viable trust level before this report can influence an actual leadership decision?**
*Assumed answer:* The system should run in shadow mode for the first quarter — generating a report in parallel with the manual QBR prep, with the DoE comparing the two. After one quarter of demonstrated accuracy, it becomes the primary input. After two quarters, it can stand alone.
*Impact on solution:* Shadow mode is not a technical feature — it is a rollout plan. The architecture supports it (the report is a separate artifact from the meeting), but the client needs to commit to the comparison discipline for it to produce useful calibration data.
...
*Assumed answer:* Likely yes — client communications may be subject to confidentiality clauses in service agreements, independent of the vendor's internal GDPR policy. Sending client emails to an external LLM API may require explicit client consent or a data processing addendum in the client contract.
*Impact on solution:* This must be clarified with legal counsel before any production deployment. The system should be designed to operate on metadata and anonymised excerpts where possible, with full email body processing only where explicit consent exists. This is distinct from general GDPR compliance and must be addressed per client contract.
